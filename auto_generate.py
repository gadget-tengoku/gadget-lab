...
def generate_summary(item, config):
    ...
    if rc >= 500:
        rv = f"楽天レビュー{rc}件・平均{ra:.1f}と母数十分な評価が安定している"
    elif rc >= 100:
        rv = f"楽天レビュー{rc}件・平均{ra:.1f}でユーザー層の評価が見える水準"
    else:
        rv = f"楽天レビュー{rc}件・平均{ra:.1f}、件数は少ないが評価点は高め"
    ...
    if price <= mid_lo:
        ...
        pp = "ボリュームゾーンの価格帯"
    if found:
        fp = f"「{found[0]}」を含む特徴がタイトルに明示されており、{config['angle']}という観点でも選択肢に入る"
    else:
        fp = f"{config['angle']}という観点では基本性能を押さえた1台"
    return f"{pp}。{rv}。{fp}。"
...
FOOTER_HTML = '''<footer>
  <div class="footer-inner">
    <div class="footer-grid">
      ...
      <div class="footer-col"><h5>About</h5><ul><li><a href="archive.html">記事一覧</a></li><li><a href="about.html">編集部</a></li><li><a href="privacy.html">プライバシーポリシー</a></li></ul></div>
